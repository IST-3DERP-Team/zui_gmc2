sap.ui.define([ 
	...
	return {        
        ...
        handleTableValueHelp: async function (oEvent) {
            ...
            await this._tableValueHelp.filterValueHelpItems(this._inputField, this);
            ...
            // if (this._sActiveTable === "attributesTab" && this._inputField === "ATTRIBCD") {
            //     await this._tableValueHelp.filterValueHelpItems(this._inputField, this);
            // }
            ...
            if (vh.length > 0) {
                Object.keys(vh[0]).filter(fItem => fItem !== "__metadata" && fItem !== "VHKey" && fItem !== "VHValue" && fItem !== "VHSelected").forEach(item => {
                    if (vhColumns !== undefined) {
                        ...
                        if (oColProp && oColProp.length > 0) {
                            if (oColProp[0].Visible) {
                                console.log(oColProp[0].ColumnName.toUpperCase())
                                aColumns.push({
                                    ColumnName: oColProp[0].ColumnName,
                                    ColumnLabel: oColProp[0].ColumnName.toUpperCase(),
                ...
                if (vhColumns !== undefined) {
                    vhColumns.forEach(item => {
                        if (item.Visible) {
                            aColumns.push({
                                ColumnName: item.ColumnName,
                                ColumnLabel: item.ColumnName.toUpperCase(),
                                Width: item.ColumnWidth,
                                DataType: item.DataType,
                                Visible: item.Visible
                            })
                    ...
                    aColumns.push({
                        ColumnName: sKey,
                        ColumnLabel: sKey.toUpperCase()
                    }) 
                    
                    if (sKey !== sValue) {
                        aColumns.push({
                            ColumnName: sValue,
                            ColumnLabel: sValue.toUpperCase()
                        }) 
                    }
                } 
            }
            ...
            aColumns.forEach(item => {
                if (oDDText[item.ColumnName.toUpperCase()] === undefined) {
                    oDDTextParam.push({CODE: item.ColumnName.toUpperCase()});
                }
            })
            ...
            aColumns.forEach(item => {
                if (!(oDDText[item.ColumnName.toUpperCase()] === undefined || oDDText[item.ColumnName.toUpperCase()] === "")) {
                    ...
                    item.ColumnLabel = item.ColumnName.toUpperCase();
                }
            })                
            ...
            if (!this._tableValueHelpDialog) {
                this._tableValueHelpDialog = sap.ui.xmlfragment(
                    "zuigmc2.view.fragments.valuehelp.TableValueHelpDialog",
                    this
                );
        ...
        filterValueHelpItems: async (sColumnName, me) => {
            var promise = new Promise((resolve, reject) => {
                if (me._sActiveTable === "attributesTab" && sColumnName === "ATTRIBCD") {
                    var sRowPath = me._inputSource.getBindingInfo("value").binding.oContext.sPath;
                    var vMattypcls = me.getView().getModel(me._sActiveTable.replace("Tab", "")).getProperty(sRowPath + '/MATTYPCLS');
